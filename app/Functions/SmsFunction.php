...
class SmsFunction
{
    ...
    public static function multiple_dash($data, $type, $requestor_no)
    {
        ...
        if ($multiple_dash > 3 && $type == "header") {
            $type = "Multiple dash";
            return $type;
        } elseif ($type == "body") {
            $orders = array_values(array_filter(preg_split("/\\r\\n|\\r|\\n/", $data)));
    ...
    {
        $valid_year = strlen(explode("-", $header)[0]) == 4;
    ...
    {
        $valid_month = strlen(explode("-", $header)[1]) == 2;
    ...
    {
        $multiple_day = current(explode("#", explode("-", $header)[1])) > 12;
    ...
    {
        $header_4 = isset(explode("-", $header)[2]) ? explode("-", $header)[2] : null;
    ...
    public static function check_alphanumerics($data, $type, $requestor_no)
    {
        if ($type == "header") {
            ...
            $year = isset(explode("-", $data)[0]) ? explode("-", $data)[0] : null;
            $month = isset(explode("-", $data)[1]) ? explode("-", $data)[1] : null;
            $day = isset(explode("-", $data)[2]) ? explode("-", $data)[2] : null;
    ...
    {
        $header_2 = isset(explode("-", $header)[0]) ? explode("-", $header)[0] : null;
        $header_3 = isset(explode("-", $header)[1]) ? explode("-", $header)[1] : null;
        $header_4 = isset(explode("-", $header)[2]) ? explode("-", $header)[2] : null;
    ...
    public static function save_sms_order($header, $body, $requestor_no, $keyword, $requestor_id)
    {
        $type = "success";
        ...
        $remarks = isset(explode("-", $header)[3]) ? explode("-", $header)[3] : "";

        $year = explode("-", $header)[0];
        $month = explode("-", $header)[1];
        $day = explode("-", $header)[2];
