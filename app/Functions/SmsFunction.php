...
class SmsFunction{
    ...
    public static function validate_body($header,$data, $requestor_no){
        ...
        $error->push( $multiple_dash_lines = SmsFunction::multiple_dash($data, $type, $requestor_no));
        $error->push( $alphanumberic_lines = SmsFunction::check_alphanumerics($data, $type, $requestor_no));
        $error->push( $missing_item_code_lines = SmsFunction::missing_code($data,$type,$requestor_no));
        $error->push( $unregistered_item_code_lines = SmsFunction::unregistered_item_code($data,$requestor_no));
        ...
        if(count($error->filter()->values()->toArray())>0){
    ...
    public static function compose_order_error (
        ...
        $type = 
        "Missing dash : ".implode(',',$missing_dash_lines)."\n".
        "Multiple dash : ".implode(',',$multiple_dash_lines)."\n".
        "Missing Item code : ".implode(',',$missing_item_code_lines)."\n".
        "Unregistered Item code : ".implode(',',$unregistered_item_code_lines)."\n".
        "Missing Qty : ".implode(',',$missing_qty_lines)."\n".
        "Qty not number : ".implode(',',$alphanumberic_lines)."\n".
        "Duplicate Orders : ".implode(',',$duplicate_lines)."\n";
    ...
    public static function check_alphanumerics ($data, $type, $requestor_no) {
        ...
        if($type == "header") {
            ...
            $order_no_validation = SmsFunction::is_alphanumeric($order_no ,'order number', $requestor_no);
    ...
    public static function duplicate($header, $data, $requestor_no){
        ...
        $transactions = $transactions->where('requestor_id',$requestor_id)
    ...
    public static function send($requestor_no,$type){
        ...
        $response = Http::withBasicAuth('rdf_vince', 'rdfPampanga@2021')->post('https://gn4l6.api.infobip.com/sms/2/text/advanced', [
            "messages" => [
              [
                "destinations" => [
                  [
