...
class CutoffController extends Controller
{
    public function index(DisplayRequest $request){
      ...
      $cut_off=$paginate?$cut_off->orderByDesc('updated_at')
      ->paginate($request->rows):$cut_off->orderByDesc('updated_at')->get();

        $is_empty = $cut_off->isEmpty();
        
        if($is_empty) return GlobalFunction::not_found(Status::NOT_FOUND);

        return GlobalFunction::display_response(Status::CUT_OFF_DISPLAY,$cut_off);
    }
    ...
    public function destroy($id){
        ...
        $cut_off = Cutoff::where('id',$id)->withTrashed()->get();
         ...
         return GlobalFunction::delete_response($message,$cut_off);
      }
}
